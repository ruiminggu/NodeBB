...
module.exports = function (Topics: any): void{
    Topics.createTopicFromPosts = async function (uid: number, title: string, pids: number[], fromTid: number) {
        if (title) {
            title = title.trim();
        }
    ...
    Topics.movePostToTopic = async function (callerUid: number, pid: number, tid: number , forceScheduled: boolean = false) {
    ...
    async function updateCategory(postData: any, toTid: number) {
